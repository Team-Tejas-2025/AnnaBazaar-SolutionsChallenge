...
String? getlatitudeandlongitude(String? latitudeandlongitude) {
  if (latitudeandlongitude == null || latitudeandlongitude.isEmpty) {
    return null;
  }
  final regex = RegExp(r'LatLng\(lat:\s*([-\d.]+),\s*lng:\s*([-\d.]+)\)');
  final match = regex.firstMatch(latitudeandlongitude);
...
String? generateProductDetails(FFUploadedFile? uploadDatagra) {
  if (uploadDatagra == null ||
      uploadDatagra.bytes == null ||
      uploadDatagra.bytes!.isEmpty) {
    return jsonEncode({"error": "No image provided"});
  }
  String base64Image = base64Encode(uploadDatagra.bytes!);
  final String apiKey = "AIzaSyDxMOZRlnK7Hod7VlBzbeUraecBy9rVD38";
  final String endpoint =
      "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0:generateContent?key=$apiKey";
    ...
    "contents": [
      {
        "parts": [
          ...
          {
            "text":
                "Generate a product name and a short product description for this image."
          }
        ]
      }
    ],
...
double subtotalprice(
  ...
  // multiply two given number and return the output
  return price * quantity.toDouble();
}

double? deleteProductfromcart(double value) {
  return value * -1;
}
...
List<DocumentReference> generateListOfUsers(
  DocumentReference followedFarmer,
  DocumentReference buyerSide,
) {
  return [followedFarmer, buyerSide];
}

List<String> generateListOfNames(
  String followedFarmerName,
  String buyerSideName,
) {
  return [followedFarmerName, buyerSideName];
}
